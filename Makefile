...
docker-publish: docker-build ## Publish the Docker image to the registry
	$(call print-target)
	docker tag $(PRODUCT):$(VERSION) $(DOCKER_REGISTRY)/$(DOCKER_REGISTRY_USER)/$(PRODUCT):latest
	docker tag $(PRODUCT):$(VERSION) $(DOCKER_REGISTRY)/$(DOCKER_REGISTRY_USER)/$(PRODUCT):$(VERSION)
	docker login $(DOCKER_REGISTRY) -u $(DOCKER_REGISTRY_USER) -p ${DOCKER_REGISTRY_PASSWORD}
	docker push $(DOCKER_REGISTRY)/$(DOCKER_REGISTRY_USER)/$(PRODUCT):latest
